...
class ExternalVariableTest(unittest.TestCase):
    def setUp(self):
        ...
        (fd, self.tempfilename_parent) = tempfile.mkstemp(
            suffix='.nc', prefix='cfdm_parent_', dir='.')
        os.close(fd)
        (fd, self.tempfilename_external) = tempfile.mkstemp(
            suffix='.nc', prefix='cfdm_external_', dir='.')
        os.close(fd)
    ...
    def test_EXTERNAL_READ(self):
        ...
        c = cfdm.read(self.combined_file, verbose=None)
    ...
    def test_EXTERNAL_WRITE(self):
        ...
        h = cfdm.read(self.tempfilename_parent, verbose=None)
