...
class H5pyArray(IndexMixin, abstract.FileArray):
    """A netCDF array accessed with `h5py`.
    ...
    * Parallelised reading is not possible.
    ...
    def _lock(self):
        ...
        Returns a lock object that prevents concurrent reads of
        netCDF-4 files, which are not currently supported by `h5py`.
...
#                dataset = self._group(dataset, groups)#
    ...
    def open(self, **kwargs):
        ...
        :Parameters:
            ...
            kwargs: optional
                Extra keyword arguments to `h5py.File`.
