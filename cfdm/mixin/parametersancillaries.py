...
class ParametersAncillaries(Parameters):
    ...
    def __bool__(self):
        ...
        """
        return super().__bool__() or bool(self.ancillaries())
    ...
    def __str__(self):
        ...
        x = ", ".join(sorted(self.ancillaries()))
    ...
    def equals(
        ...
        * the names of ancillary constructs are the same.
        ...
        **Examples**
        ...
        if set(ancillaries0) != set(ancillaries1):
            logger.info(
                f"{self.__class__.__name__}: Different ancillary terms "
                f"({set(ancillaries0)} != {set(ancillaries1)})"
            )  # pragma: no cover
            return False
